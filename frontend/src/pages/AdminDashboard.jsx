...
import {
  ...
  Save, Loader2, Download, MessageSquarePlus, ChevronDown, ChevronUp, Clock
} from 'lucide-react';
...
const NotesPanel = ({ lead, onNoteAdded }) => {
  ...
  return (
    <div className="mt-2">
      <button
        onClick={() => setOpen(p => !p)}
        className="flex items-center gap-1.5 text-xs text-text-muted hover:text-accent-ice transition-colors"
      >
          ...
          >
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-void border border-white/10 rounded-lg px-3 py-1.5 text-xs text-text-white outline-none focus:border-accent-violet/50 placeholder:text-text-muted"
              />
              <button
                onClick={handleAddNote}
                disabled={saving || !text.trim()}
                className="p-1.5 rounded-lg bg-accent-violet/20 border border-accent-violet/30 text-accent-violet-light hover:bg-accent-violet/30 transition-colors disabled:opacity-40"
              >
...
const AdminDashboard = () => {
  ...
  const handleRefreshAfterNote = async (leadId) => {
    try {
      const response = await api.get('/admin/leads');
  ...
  const exportCSV = async () => {
    try {
      ...
      const res = await fetch(`${baseUrl}/admin/leads/export/csv`, {
      ...
      a.href = url;
      a.download = `brixton-leads-${Date.now()}.csv`;
      a.click();
  ...
  return (
    <div className="min-h-screen bg-void flex flex-col md:flex-row text-text-white selection:bg-accent-violet/30">
      {/* Sidebar */}
      <aside className="w-full md:w-64 bg-base border-r border-border-subtle p-6 flex flex-col flex-shrink-0 z-10">
        ...
        <nav className="flex-1 space-y-2 font-body">
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-[rgba(255,255,255,0.08)] text-text-white font-medium border border-[rgba(255,255,255,0.05)]">
            <LayoutDashboard size={18} className="text-accent-ice" /> Dashboard
          </button>
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-text-secondary hover:bg-[rgba(255,255,255,0.03)] hover:text-text-white transition-colors border border-transparent">
            <Users size={18} /> Leads
          </button>
        </nav>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-8 gap-4">
            ...
            <div className="flex flex-wrap gap-3 font-body text-xs">
          ...
          <div className="bg-card border border-border-medium p-4 md:p-6 rounded-3xl shadow-2xl relative backdrop-blur-sm">
            ...
            <div className="flex flex-col md:flex-row justify-between mb-6 gap-3 font-body">
              <div className="flex flex-wrap items-center gap-3">
                <div className="relative w-full md:w-56">
                  ...
                  <input
                    ...
                    className="w-full bg-[rgba(255,255,255,0.03)] border border-border-subtle rounded-lg pl-9 pr-4 py-2 text-sm text-text-white focus:border-accent-ice outline-none"
                  />
                </div>
                <select
                  value={filter}
              ...
              <div className="flex gap-2">
                <button
                  onClick={exportCSV}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-accent-gold-light/10 border border-accent-gold-light/20 hover:bg-accent-gold-light/20 text-sm font-medium text-accent-gold-light transition-colors"
                >
                  <Download size={15} /> Export CSV
                </button>
                <button
                  onClick={() => fetchLeads(true)}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[rgba(255,255,255,0.03)] border border-border-subtle hover:bg-[rgba(255,255,255,0.08)] text-sm font-medium transition-colors"
                >
              ...
              <table className="w-full text-left border-collapse min-w-[850px] font-body">
                <thead>
                  <tr className="border-b border-border-subtle text-text-secondary text-xs uppercase tracking-wider">
                    ...
                    <th className="pb-3 px-3 font-semibold w-24">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <AnimatePresence>
                    {!loading && filteredLeads.map(lead => (
                      <motion.tr
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="border-b border-border-subtle/50 hover:bg-[rgba(255,255,255,0.02)] transition-colors group"
                      >
                        {/* Name */}
                        ...
                        {/* Service */}
                        <td className="py-4 px-3 align-top">
                          <span className="inline-block px-2.5 py-1 bg-[rgba(255,255,255,0.03)] border border-border-subtle rounded-lg text-xs font-mono text-accent-ice">
                            {lead.service}
                          </span>
                        </td>
                        ...
                        <td className="py-4 px-3 align-top max-w-xs">
                          {editingId === lead.id ? (
                            <textarea
                              ...
                              className="w-full bg-void border border-accent-violet/50 rounded p-2 text-xs text-text-white resize-none h-20 outline-none"
                              placeholder="Admin comment..."
                            />
                          ) : (
                            <div className="space-y-2">
                              {lead.message && (
                                <p className="text-xs text-text-secondary line-clamp-2" title={lead.message}>{lead.message}</p>
                              )}
                              {lead.comment && (
                                <p className="text-xs text-accent-violet-light italic">{lead.comment}</p>
                              )}
                        ...
                        <td className="py-4 px-3 align-top">
                          {editingId === lead.id ? (
                            <select
                              ...
                              className="bg-void border border-border-subtle rounded p-1 text-xs text-text-white outline-none"
                            >
                              {Object.entries(STATUS_CONFIG).map(([k, v]) => (
                                <option key={k} value={k} className="bg-void">{v.label}</option>
                              ))}
                        ...
                        <td className="py-4 px-3 align-top">
                          {editingId === lead.id ? (
                            <div className="flex gap-2">
                              <button onClick={() => saveEdit(lead.id)} className="p-1.5 rounded bg-green-500/20 text-green-400 hover:bg-green-500/30 border border-green-500/30 transition-colors" title="Save">
                                <Save size={14} />
                              </button>
                              <button onClick={() => setEditingId(null)} className="p-1.5 rounded bg-rose-500/20 text-rose-400 hover:bg-rose-500/30 border border-rose-500/30 transition-colors" title="Cancel">
                                <X size={14} />
                              </button>
                            </div>
                          ) : (
                            <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => startEdit(lead)} className="p-1.5 rounded bg-[rgba(255,255,255,0.03)] border border-border-subtle hover:text-accent-ice hover:border-accent-ice/50 transition-colors" title="Edit">
                                <Edit3 size={14} />
                              </button>
                            </div>
                          )}
